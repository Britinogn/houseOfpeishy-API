...
const sendNotification = async (to, message, preferredMethod = 'sms') => {
  try {
    // Format number for Nigeria (Termii requires country code)
    ...
    //  Send SMS
    const smsResponse = await axios.post('https://v3.api.termii.com/api/sms/send', {
      api_key: process.env.TERMII_API_KEY,
      to: phoneNumber,
      from: process.env.TERMII_SENDER_ID, // must be an approved Sender ID
      sms: message,
      type: 'plain',
      channel: 'generic'
    });
